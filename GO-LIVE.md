...
Your site already works today: the enquiry form falls back to emailing you directly, so no lead is ever lost. These steps switch on the *full* flow (email delivery, a scheduler button, and a pay-now button). Total time: ~15–20 minutes.
...
4. Add these env vars in Vercel:
   ...
   | `LEAD_TO_EMAIL` | `yp.wangt@gmail.com` (or your business inbox) |
   | `LEAD_FROM_EMAIL` *(optional)* | `North Star Pacific <hello@yourdomain.com>` |
...
## 2. Scheduler — the "Book a call" button

1. Create a free account at **[cal.com](https://cal.com)** (or Calendly).
2. Make an event type, e.g. **"Growth Audit — intro call"**, 15 min.
3. Copy its public link, e.g. `https://cal.com/your-handle/growth-audit`.
...
✅ The "Book a call" button appears automatically wherever it's wired in [config.ts](src/config.ts).

---

## 3. Paid audit — the "Start your audit" button

1. In **[Stripe](https://dashboard.stripe.com)**, create a Product: **"Business Technology Growth Audit"**, price **$1,500**.
...
✅ The "Start your audit" button appears automatically.
...
- Swap `yp.wangt@gmail.com` if you move to a business inbox.
...
- [ ] Open the live site, scroll to **"Book your audit"**.
- [ ] Submit the form with a test entry → you see the success screen **and** an email lands in `LEAD_TO_EMAIL`.
- [ ] "Book a call" opens your scheduler; "Start your audit" opens Stripe checkout.
- [ ] `/privacy.html` and `/terms.html` load and show the right email.

---

### Want me to wire it instead?
Once you've created the accounts, just paste me the **booking link** and **payment link** (and confirm the contact email), and I'll set the config, redeploy locally to verify, and run the smoke test with you.

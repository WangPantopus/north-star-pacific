...
Marketing site and lead flow for North Star Pacific, an AI enablement and implementation partner for growing companies.
...
- **Booking section** — the `BookAudit` component in `src/App.tsx` (anchor `#book`): the lead-capture form plus optional "Book an AI Opportunity Call" button. Every site CTA points here.
...
> ⚠️ The site currently uses `https://north-star-pacific.vercel.app/` in `index.html`, `public/sitemap.xml`, and `public/robots.txt`. Swap to your custom domain in those files when you add one.
...
| `templates/` | Sales docs: Proposal, SOW, AI Opportunity Audit report, Lead triage checklist |
